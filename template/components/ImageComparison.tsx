...
const ImageComparison: React.FC<ImageComparisonProps> = ({ beforeUrl, afterUrl, onDownload }) => {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden border border-slate-200">
      ...
      <div 
        ref={containerRef}
        className="relative w-full h-[500px] bg-[url('https://bg.site/checkerboard.png')] bg-repeat overflow-hidden cursor-col-resize select-none group"
      ...
      >
        ...
        {/* Labels */}
        <div className="absolute bottom-4 left-4 bg-black/50 text-white text-xs px-2 py-1 rounded pointer-events-none">Original</div>
        <div className="absolute bottom-4 right-4 bg-indigo-600/80 text-white text-xs px-2 py-1 rounded pointer-events-none">Enhanced</div>
      </div>
    </div>
  );
};
