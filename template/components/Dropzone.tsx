...
import { UploadCloud, Image as ImageIcon, AlertCircle, FileUp } from 'lucide-react';
...
const Dropzone: React.FC<DropzoneProps> = ({ onFilesSelected, disabled, compact = false }) => {
      ...
      <div className="flex flex-col items-center justify-center text-center space-y-4 pointer-events-none select-none">
        ...
        {!compact && (
          <div className="space-y-2 max-w-xs mx-auto">
            <h3 className={`font-bold text-slate-900 transition-colors ${isDragging ? 'text-indigo-700' : 'group-hover:text-indigo-600'} text-xl`}>
              {isDragging ? 'Drop to upload' : 'Click or drag images'}
            </h3>
            <p className="text-slate-500 text-sm font-medium leading-relaxed">
              Support for JPG, PNG, and WEBP
              <span className="block text-xs text-slate-400 mt-1 font-normal">Up to 5MB per file • Batch processing available</span>
            </p>
          </div>
        )}
      </div>
