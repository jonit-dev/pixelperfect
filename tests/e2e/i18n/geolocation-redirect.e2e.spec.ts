...
test.describe('Geolocation Auto-Redirect', () => {
  ...
  test.describe('Cookie Override', () => {
    ...
    test('should update cookie when user manually switches language', async ({
      page,
      context,
    }) => {
      ...
      const switcherButton = page.locator('footer').getByRole('button').filter({
    ...
    test('should not redirect when locale cookie matches current URL', async ({ page, context }) => {
      ...
      const responseCount = await page.evaluate(() => performance.getEntriesByType('navigation').length);
  ...
  test.describe('hreflang Tags', () => {
    test('should include all 7 locale variants in hreflang', async ({ page }) => {
      ...
      // Should have hreflang tags for all supported locales + x-default
      expect(hreflangTags.length).toBeGreaterThanOrEqual(7);
      ...
      // Verify all supported locales exist
      for (const locale of ['en', 'es', 'pt', 'de', 'fr', 'it', 'ja']) {
        const hreflang = await page.locator(`link[rel="alternate"][hreflang="${locale}"]`).count();
        expect(hreflang).toBeGreaterThan(0);
      }
    });

    test('should have correct hreflang URLs for each locale', async ({ page }) => {
      ...
      // Check that hreflang URLs are correct
      const portugueseHreflang = page.locator('link[rel="alternate"][hreflang="pt"]');
      const href = await portugueseHreflang.getAttribute('href');

      expect(href).toContain('/pt/tools/ai-image-upscaler');
    });

    test('should have self-referencing canonical', async ({ page }) => {
      // Navigate to a localized page
      ...
      expect(href).toContain('/es/tools/ai-image-upscaler');
      expect(href).not.toContain('/en/');
    });

    test('should have x-default pointing to English', async ({ page }) => {
      // Navigate to a non-English page
      ...
      expect(href).toContain('/tools/ai-image-upscaler');
      expect(href).not.toContain('/fr/');
      expect(href).not.toMatch(/\/(es|pt|de|it|ja)\//);
    });
  });
