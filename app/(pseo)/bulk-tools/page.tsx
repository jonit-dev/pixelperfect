...
export const metadata = generateCategoryMetadata('bulk-tools');

export default async function BulkToolsHubPage() {
  const pages = await getAllBulkToolsPages();

  return (
    <div className="container mx-auto px-4 py-12">
      <h1 className="text-4xl font-bold mb-6">Bulk Image Processing Tools</h1>
      <p className="text-xl text-text-secondary mb-12">
        Process multiple images at once with our powerful batch processing tools
      </p>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {pages.map(page => (
          <Link
            key={page.slug}
            href={`/bulk-tools/${page.slug}`}
