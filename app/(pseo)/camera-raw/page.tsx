...
export const metadata = generateCategoryMetadata('camera-raw');

export default async function CameraRawHubPage() {
  const pages = await getAllCameraRawPages();

  return (
    <div className="container mx-auto px-4 py-12">
      <h1 className="text-4xl font-bold mb-6">Camera RAW Image Enhancement</h1>
      <p className="text-xl text-text-secondary mb-12">
        Professional RAW file processing and upscaling for all major camera brands
      </p>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {pages.map(page => (
          <Link
            key={page.slug}
            href={`/camera-raw/${page.slug}`}
