...
${alternatives
  .map(
    alternative => `  <url>
    <loc>${BASE_URL}/alternatives/${alternative.slug}</loc>
    <lastmod>${alternative.lastUpdated}</lastmod>
    <changefreq>weekly</changefreq>
    <priority>0.75</priority>${
      alternative.ogImage
        ? `
    <image:image>
      <image:loc>${alternative.ogImage}</image:loc>
      <image:title>${alternative.title}</image:title>
    </image:image>`
        : ''
    }
  </url>`
