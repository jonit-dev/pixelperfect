...
export function BulkImageCompressor(): React.ReactElement {
  const t = useTranslations('tools-ui.bulkCompressor');
  const tCommon = useTranslations('tools-ui.common');
  const tShared = useTranslations('tools-ui.shared');
      ...
      try {
        ...
        if (result) {
          ...
          setImages(prev =>
            prev.map(img =>
              img.id === imageId
                ? { ...img, isProcessing: false, error: t('errors.failedToCompress') }
                : img
            )
          );
        }
        ...
        setImages(prev =>
          prev.map(img =>
            img.id === imageId
              ? { ...img, isProcessing: false, error: t('errors.compressionFailed') }
              : img
          )
        );
      }
  ...
  return (
    <div className="w-full max-w-5xl mx-auto">
      <div className="p-6 border-2 border-border bg-surface shadow-lg rounded-xl">
        ...
        {images.length === 0 && (
          <div className="border-2 border-dashed border-border rounded-xl p-12 text-center hover:border-accent/50 transition-colors">
            ...
            <h3 className="text-xl font-semibold text-text-primary mb-2">{t('dropImages')}</h3>
            <p className="text-text-secondary mb-4">{t('uploadLimit')}</p>
            <button
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-6 p-4 bg-surface-light rounded-lg">
          ...
          <div>
            ...
            />
            <p className="text-xs text-text-muted mt-1">{t('leaveEmpty')}</p>
          </div>
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2 bg-surface border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-accent text-text-primary"
            >
              <option value="jpeg">{t('formatOptions.jpeg')}</option>
              <option value="webp">{t('formatOptions.webp')}</option>
              <option value="png">{t('formatOptions.png')}</option>
            </select>
          </div>
        </div>
          ...
          <div className="mb-6 p-4 bg-surface-light rounded-lg">
            <div className="grid grid-cols-4 gap-4 text-center">
              <div>
                ...
                <p className="text-xs text-text-secondary">{t('images')}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-xs text-text-secondary">{t('avgReduction')}</p>
              </div>
              <div>
              ...
              >
                {isProcessingAll ? (
                  <>
                    ...
                    {t('processing', { processed: processedCount, total: images.length })}
                  </>
                ) : (
                  t('compressImages', {
                    count: images.length,
      ...
      <div className="mt-4 p-4 bg-surface rounded-lg text-sm text-text-secondary">
        <p className="font-medium mb-2">{t('tips.title')}</p>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-surface border border-border rounded-2xl max-w-md w-full p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <p className="text-text-secondary text-center mb-2">
              {t('success.saved', {
                ratio: avgCompressionRatio,
                count: images.length,
                plural: images.length > 1 ? 's' : '',
              })}
            </p>
            <p className="text-text-muted text-center text-sm mb-6">{t('success.downloadBelow')}</p>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h4 className="font-semibold text-text-primary mb-1">
                    {t('success.needHigherResolution')}
                  </h4>
                  <p className="text-sm text-text-secondary">{t('success.upscalerDescription')}</p>
                </div>
              </div>
            </div>
