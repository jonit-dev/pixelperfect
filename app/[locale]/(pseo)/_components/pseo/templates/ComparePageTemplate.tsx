        ...
        <div className="pt-6 pb-4">
          <BreadcrumbNav
            items={[
              ...
              { label: t('common.comparisons'), href: '/compare' },
        ...
        <article>
          ...
          <div className="relative">
            <div className="absolute -top-4 left-0">
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-warning/20 text-warning text-sm font-semibold rounded-full">
                ...
                {t('templates.comparison.badge.label')}
              </span>
            </div>
            <div className="pt-8">
              <HeroSection
                h1={data.h1}
                intro={data.intro}
                ctaText={t('common.tryFree', { appName: clientEnv.APP_NAME })}
                ctaUrl="/?signup=1"
                pageType="comparison"
                slug={data.slug}
              />
            ...
            <FadeIn delay={0.3}>
              <section className="py-12">
                <h2 className="text-2xl md:text-3xl font-bold text-center mb-8">
                  {t('templates.comparison.table.headToHead')}
                </h2>
                    ...
                    <thead className="bg-surface">
                      <tr>
                        <th className="px-6 py-4 text-left text-sm font-semibold text-primary">
                          {t('templates.comparison.table.feature')}
                        </th>
                          ...
                          >
                            {product.isRecommended && (
                              <div className="inline-flex items-center gap-1 px-2 py-1 bg-accent text-white text-xs font-bold rounded-full mb-2">
                                ...
                                {t('templates.comparison.table.best')}
                              </div>
                            )}
                ...
                <div className="max-w-3xl mx-auto bg-gradient-to-br from-accent/10 to-accent/5 border-2 border-accent/20 rounded-xl p-8">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h2 className="text-2xl font-bold">
                      {t('templates.comparison.verdict.title')}
                    </h2>
                  </div>
                    ...
                    {data.verdict.winner && (
                      <div className="mt-4 p-4 bg-surface rounded-lg border border-accent/20">
                        <div className="font-semibold text-primary mb-1">
                          {t('templates.comparison.verdict.winner')}
                        </div>
          ...
          {data.products && data.products.length > 0 && (
            <FadeIn delay={0.5}>
              <section className="py-12">
                <h2 className="text-2xl md:text-3xl font-bold text-center mb-8">
                  {t('templates.comparison.reviews.title')}
                </h2>
          ...
          <div className="py-8">
            <FadeIn>
              <CTASection
                title={t('common.readyToTry')}
                description={t('common.experienceDifference', { appName: clientEnv.APP_NAME })}
                ctaText={t('common.tryFree', { appName: clientEnv.APP_NAME })}
                ctaUrl="/?signup=1"
                pageType="comparison"
                slug={data.slug}
            ...
            <FadeIn delay={0.6}>
              <section className="py-8 border-t border-border">
                <h2 className="text-2xl font-bold mb-6">
                  {t('templates.comparison.related.title')}
                </h2>
