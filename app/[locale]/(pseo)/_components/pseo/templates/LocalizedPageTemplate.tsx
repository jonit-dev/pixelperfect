    ...
    <div className="min-h-screen bg-base relative">
      ...
      <div className="relative max-w-5xl mx-auto px-6 sm:px-8 lg:px-12 py-12">
        ...
        <div className="text-center py-12">
          <h1 className="text-3xl font-bold text-text-primary mb-4">Page Not Available</h1>
          <p className="text-lg text-text-secondary">
            This page is currently only available in English. Please use the link above to view the
            English version.
          </p>
        </div>
      </div>
...
export function withLocalization<T extends { data: PSEOPage }>(
  ...
  return function LocalizedWrapper(props: T & { locale: Locale; slug: string }): ReactElement {
    const { locale, slug, data, ...rest } = props;

    return (
      <LocalizedPageTemplate locale={locale} pageData={data} category={category} slug={slug}>
