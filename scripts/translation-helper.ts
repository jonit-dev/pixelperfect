...
// Check if an array should stay inline (short, primitives only)
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function shouldArrayBeInline(arr: any[]): boolean {
  if (arr.length === 0) return true;
  if (arr.length > 6) return false;

  // Only primitives allowed for inline
  const allPrimitives = arr.every(
    item => typeof item === 'string' || typeof item === 'number' || typeof item === 'boolean'
  );
  if (!allPrimitives) return false;

  // Check total string length when joined
  const joined = JSON.stringify(arr);
  return joined.length <= 80;
}

// Custom JSON stringify that keeps short arrays inline
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function prettyStringify(data: any, indent: number = 0): string {
  ...
  if (Array.isArray(data)) {
    ...
    if (shouldArrayBeInline(data)) {
      // Format with spaces after commas for readability
      return '[' + data.map(item => JSON.stringify(item)).join(', ') + ']';
    }

    const items = data.map(item => childSpaces + prettyStringify(item, indent + 1));
    return '[\n' + items.join(',\n') + '\n' + spaces + ']';
  }
  ...
  if (typeof data === 'object') {
    ...
    const items = keys.map(key => {
      const value = prettyStringify(data[key], indent + 1);
...
// Write JSON file with pretty formatting (keeps short arrays inline)
// Preserves original file's trailing newline behavior
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function writeJsonFile(filePath: string, data: any): Promise<void> {
  // Check if original file had trailing newline
  let hasTrailingNewline = true; // Default to having one for new files
  if (fs.existsSync(filePath)) {
    const original = await fs.promises.readFile(filePath, 'utf-8');
    hasTrailingNewline = original.endsWith('\n');
  }

  const content = prettyStringify(data) + (hasTrailingNewline ? '\n' : '');
...
// Command: get-all - Get ALL untranslated entries for a file (no batching)
async function getAll(locale: string, fileName: string): Promise<void> {
  if (locale === SOURCE_LOCALE) {
    console.error(JSON.stringify({ error: 'Cannot get entries for English source locale' }));
    process.exit(1);
  }
  ...
  const output = {
    ...
    file,
    targetLanguage: getLanguageName(locale),
    totalEntries: allNeeded.length,
    entries: allNeeded.map(e => ({
      key: e.key,
      english: e.value,
    })),
    responseFormat: {
      description: 'Return a JSON array of objects with "key" and "value" fields',
      example: [{ key: allNeeded[0]?.key || 'example.key', value: '<translated text>' }],
    },
  };
...
// Command: stats-json - Machine-readable stats
async function showStatsJson(locale: string): Promise<void> {
  const sourceDir = path.join(LOCALES_DIR, SOURCE_LOCALE);
  const targetDir = path.join(LOCALES_DIR, locale);
  ...
  for (const file of files) {
    ...
    try {
      ...
      const diff = compareWithSource(sourceEntries, targetEntries);
      const total = sourceEntries.length;
      const progress = total > 0 ? Math.round((diff.translated / total) * 100) : 100;
      ...
      results.push({
        ...
        untranslated: diff.untranslated.length,
        translated: diff.translated,
        progress,
      });
    } catch (error) {
  ...
  const output = {
    ...
    summary: {
      ...
      untranslated: grandUntranslated,
      translated: grandTranslated,
      needsWork: grandMissing + grandUntranslated,
      progress: grandTotal > 0 ? Math.round((grandTranslated / grandTotal) * 100 * 10) / 10 : 100,
    },
    files: results,
  };
...
// Command: priority-files - List files sorted by untranslated count
async function priorityFiles(locale: string): Promise<void> {
  const sourceDir = path.join(LOCALES_DIR, SOURCE_LOCALE);
  const targetDir = path.join(LOCALES_DIR, locale);
  ...
  // Sort by needsWork descending
  fileStats.sort((a, b) => b.needsWork - a.needsWork);

  console.log(`\n# Priority Files for ${getLanguageName(locale)}\n`);
  console.log('Files sorted by work needed (highest first):\n');
  console.log('| # | File | Needs Work | Missing | Untranslated | Total |');
  console.log('|---|------|------------|---------|--------------|-------|');

  fileStats.forEach((f, i) => {
    const shortName = f.file.length > 28 ? f.file.slice(0, 25) + '...' : f.file;
    console.log(
      `| ${String(i + 1).padStart(2)} | ${shortName.padEnd(28)} | ${String(f.needsWork).padStart(10)} | ${String(f.missing).padStart(7)} | ${String(f.untranslated).padStart(12)} | ${String(f.total).padStart(5)} |`
    );
  });

  const totalWork = fileStats.reduce((sum, f) => sum + f.needsWork, 0);
  console.log(`\nTotal entries needing work: ${totalWork} across ${fileStats.length} files`);
}
...
Commands:
  ...
  get-all <locale> <file>                   Get ALL untranslated entries (for bulk work)
  get-batch <locale> <file> [size] [offset] Get batch of entries (default: 50)
  apply <locale> <batch-file>               Apply translations from batch file
  apply-inline <locale> <file> '<json>'     Apply translations inline
  sync <locale> [file]                      Copy missing keys from English
  validate <locale> [file]                  Validate JSON structure
  stats <locale>                            Show translation statistics (table)
  stats-json <locale>                       Show translation statistics (JSON)
  priority-files <locale>                   List files sorted by work needed
  list-files <locale>                       List all translation files

Examples:
  # See priority files for Italian
  npx tsx scripts/translation-helper.ts priority-files it

  # Get ALL untranslated entries for bulk translation
  npx tsx scripts/translation-helper.ts get-all it format-scale.json

  # Get 50 entries needing translation (default batch size)
  npx tsx scripts/translation-helper.ts get-batch de social-media-resize.json

  # Get larger batch of 100 entries
  npx tsx scripts/translation-helper.ts get-batch de social-media-resize.json 100

  # Apply inline translations (from LLM output)
  npx tsx scripts/translation-helper.ts apply-inline de social-media-resize.json '[{"key":"pages[0].title","value":"Bild für Pinterest skalieren"}]'

  # Get machine-readable stats
  npx tsx scripts/translation-helper.ts stats-json it
...
LLM Bulk Workflow:
  1. priority-files → see what needs most work
  2. get-all → get ALL entries for a file
  3. Translate in batches of 50-100
  4. apply-inline → apply translations
  5. validate → confirm no JSON errors
  ...
  try {
    switch (command) {
      ...
      case 'diff-json':
        ...
        break;
      case 'get-batch':
        await getBatch(args[1], args[2], parseInt(args[3]) || 50, parseInt(args[4]) || 0);
        break;
      case 'get-all':
        await getAll(args[1], args[2]);
        break;
      case 'apply':
      ...
      case 'stats':
        ...
        break;
      case 'stats-json':
        await showStatsJson(args[1]);
        break;
      case 'priority-files':
        await priorityFiles(args[1]);
        break;
      case 'list-files':
