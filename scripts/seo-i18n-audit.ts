 ...
 */
const EXPECTED_TRANSLATIONS = ['common.json', 'tools.json', 'formats.json', 'free.json', 'guides.json'];
 ...
 */
function validateJson(filePath: string): { valid: boolean; error?: string; content?: ITranslationFile } {
  try {
...
function auditLocaleCoverage() {
  ...
  if (warnings.length === 0) {
    ...
    addResult('Locale Coverage', 'pass', 'All categories have translations for all supported locales');
  } else {
...
function auditDuplicateKeys() {
  ...
  if (duplicatesFound.length === 0) {
    ...
    addResult('Duplicate Keys', 'fail', 'Duplicate keys found in translation files', duplicatesFound);
  }
}
...
function generateSummary() {
  ...
  const groupedByCategory = auditResults.reduce((acc, result) => {
